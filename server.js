	...
	try {
		...
		if (req.method == 'GET') {
			...
			if (parsed_url.pathname == "/play") {
				...
				const inventory = parsed_url.query.c
					.split(' ')
					.map(x => parseInt(x))
				...
				const show_data = {
					res, game: game[0].name, states, location: locationID,
					inventory, description: location[0].description
				};
				...
				if (split_go_to) {
					...
					if (obj.length == 1) {
						...
						await show(show_data, 
							`There is no ${split_pick_up[1]} here.`);
					}
				} else if (split_use) {
					...
					switch (parsed_url.query.type) {
						...
						case "path": {
							...
							var text = await query(`
								SELECT text FROM ??
								WHERE ID = ?`, [table, parsed_url.query.id]);
